...
def test_accounting_buy_sell_realized_pnl() -> None:
    ...
    fill_buy = Fill(symbol="T", side=Side.BUY, qty=1.0, fill_price=100.0, fees=0.0, slippage=0.0, timestamp=pd.Timestamp.now("UTC"), bar_index=0)
    fill_sell = Fill(symbol="T", side=Side.SELL, qty=1.0, fill_price=110.0, fees=0.0, slippage=0.0, timestamp=pd.Timestamp.now("UTC"), bar_index=1)
