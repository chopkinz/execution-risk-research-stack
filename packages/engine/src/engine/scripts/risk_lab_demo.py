"""Run risk-engine-lab demo: policy-driven risk evaluation and rejection audit."""
...
def run_risk_lab_demo(out: Path) -> Path:
    ...
    portfolio = DemoPortfolio(
        trades_today=0, daily_pnl=0.0, equity=100000.0, drawdown_pct=0.0, exposure=0.0
    )
    reasons: list[str] = []
    for i in range(6):
    ...
    ax.set_title("Equity Curve (Risk Lab Demo)")
    fig.tight_layout()
    ...
    plt.close(fig)
    report_lines = [
        "# risk-engine-lab Demo Report",
        "",
        "- Offline deterministic risk evaluation demo (policy-driven limits).",
    ]
